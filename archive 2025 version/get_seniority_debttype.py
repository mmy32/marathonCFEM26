...
def build_final_dataset_with_seniority_and_sector(
    ...
    llm_model : str
        OpenAI model name used for LLM sector classification (e.g., "gpt-4.1").

    batch_size : int
        Number of borrower names sent per LLM call.

    openai_api_key : Optional[str]
        OpenAI API key. If None, the function will read from environment variable OPENAI_API_KEY.
...
ALLOWED SECTORS (choose exactly one per borrower):
- Technology
- Healthcare
- Industrials
- Transportation
...
3. Be conservative: if you are not reasonably sure, use
   "Other / Unknown / Unresolved" with low confidence.
4. You may use your own knowledge about companies and sectors, or use external sources if needed (unknown company), web search, databases, etc.
...
"""
    ...
    def classify_batch_with_llm(
        ...
        """
        Call OpenAI for a batch of borrower names.
        ...
        try:
            ...
            print("OpenAI API error on this batch:", e)
    ...
    if verbose:
        print(f"\nRunning OpenAI LLM on {n} unique unclassified borrowers in {n_batches} batches...")
...
if __name__ == "__main__":
    ...
    build_final_dataset_with_seniority_and_sector(
        ...
        llm_model="gpt-4.1",
        batch_size=32,
        openai_api_key=None,  # uses env var OPENAI_API_KEY
        verbose=True,
    )