...
def run_preprocess(
    ...
    df_all = pd.read_csv(in_csv, engine="pyarrow")
    logger.info(f"Loaded: {in_csv} (rows={len(df_all)}, cols={len(df_all.columns)})")

    # calendar quarter
    df_all = add_calendar_quarter_columns(df_all, period_col="period")

    # prefix merge
    before_cols = len(df_all.columns)
    df_merged = prefix_merge_columns(df_all)
    after_cols = len(df_merged.columns)
    logger.info(f"Prefix-merge cols: {before_cols} -> {after_cols} (reduced {before_cols - after_cols})")

    # diagnostics
    if cfg.write_stats_csv:
        stats = nonnull_stats(df_merged)
        stats_path = out_csv.with_name("preprocess_nonnull_stats.csv")
        stats.to_csv(stats_path, index=False)
        logger.info(f"Wrote stats -> {stats_path}")

    # keep analysis cols
    df = select_analysis_columns(df_merged)

    # filter quarter range
    if "cal_q" in df.columns:
        df["cal_q"] = df["cal_q"].astype(str)
        df = df[df["cal_q"].between(cfg.quarter_from, cfg.quarter_to)].copy()

    # context type
    df["context_type"] = classify_context(df)

    # share filter
    if cfg.only_no_shares and cfg.keep_share_col in df.columns:
        df = df[df[cfg.keep_share_col].isna()].copy()

    # drop amounts_only
    if cfg.drop_amounts_only:
        df = df[df["context_type"] != "amounts_only"].copy()

    # normalize
    df = normalize_interest_columns(df)

    df.to_csv(out_csv, index=False)
    logger.info(f"Wrote clean output -> {out_csv} (rows={len(df)}, cols={len(df.columns)})")
    return df
